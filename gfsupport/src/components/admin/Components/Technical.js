...
import {
  ...
  function AddNewAnswer(props) {
   ...
   const handleSubmit = e =>{
     ...
     axios.post('http://127.0.0.1:8000/api/other', formData,{
      headers: {
...
export const Technical = () => {
    ...
    const renderPagination = () =>{
      ...
      const handleFetch = async () =>{
        await axios.get(`http://127.0.0.1:8000/api/other?page=${count}`,{
          headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `Bearer ${window.localStorage.getItem("token")}`
          }})
    ...
    return (
            ...
            <span className="d-flex justify-content-between p-4 m-2">
                <Heading className="text-center" fontFamily="Fira sans">Probleme Technique  </Heading>
